      ...
      tap({
        next: (result: ProductModel) => {
          ...
          const products = result.data.filter(product => 
              ...
              (product?.categories.length && product?.categories?.map(category => category._id).includes(action?.payload?.['category_ids']))
          );
          ctx.patchState({
            ...state,
      ...
      tap({
        next: results => {
          ...
          if(result) {
            ...
            const categoryIds = [...result?.categories?.map(category => category._id)];
